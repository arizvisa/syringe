...
def Default(*args, **kwds):
    return Managed(*args, **kwds)

class MemoryManager(object):
    allocator = debugger = None
    loaded = committed = dict

    def __init__(self, debugger):
        self.debugger = debugger
        self.allocator = pageexec.Default( self.debugger )
        self.loaded = {}
        self.committed = {}
...
class Managed(MemoryManager):
    # XXX: i feel like this was largely influenced by my phk research from like 8 years ago
    arenas = dict
    allocations = dict    # lookup all page occupancy

    def __init__(self, *args, **kwds):
        super(Managed, self).__init__(*args, **kwds)
    ...
    def alloc(self, size):
        ...
        if size < pagesize>>1:
            ...
            res = self.__alloc_pages((size+1)/pagesize)
    ...
    def __alloc_arena(self, elementsize, pages=1):
        ...
        res = bitmap.new( size / elementsize )
...
if __name__ == '__main__':
    import sys
    import memorymanager, debug

    debugger = debug.DefaultDebugger()
    debugger.attach(int(sys.argv[1], 0x10))

    v = memorymanager.Default(debugger)
