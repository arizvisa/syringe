...
class Atom(pstruct.type):
    def __data(self):
        ...
        t = AtomType.get(type, type=type, __name__='Unknown<{!r}>'.format(type), length=size)
...
class stsd(EntriesAtom):
    ...
    class Audio(pstruct.type):
        _fields_ = [
            ...
            (lambda s: MediaAudio.get(s['Version'].li.int(), type=s['Version'].int()), 'Versioned'),
        ]
    ...
    class Video(pstruct.type):
        _fields_ = [
            ...
            (lambda s: MediaVideo.get(s['Version'].li.num(), type=s['Version'].int()), 'Versioned'),
        ]
