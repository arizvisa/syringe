...
class IMsoInkData(pstruct.type):
    _fields_ = [
        ...
        (uint4, 'cbBlob'),
        (lambda self: dyn.block(self['cbBlob'].li.int()), 'data'),
    ]
...
class IHlink(pstruct.type):
    _fields_ = [
        ...
        (undefined, 'hyperlink'), # FIXME: [MS-OSHARED] Hyperlink
    ]
...
class equationXML(uint4):
    type = 0x030c
    complex = lambda elfs: dyn.clone(pstr.string, length=self.int())
...
class wzName(uint4):
    type = 0x0380
    complex = lambda self: dyn.clone(pstr.string, length=self.int())
...
class pihlShape(uint4):
    type = 0x0382
    complex = lambda self: dyn.block(self.int())

@OfficeArtFOPTEOP.define
class pWrapPolygonVertices(uint4):
    type = 0x0383
    complex = lambda self: dyn.clone(IMsoArray, _object_=POINT, blocksize=lambda _: self.int())
...
class tableRowProperties(uint4):
    type = 0x03a0
    complex = lambda self: dyn.clone(IMsoArray, _object_=sint4, blocksize=lambda _: self.int())
...
class pSegmentInfo(uint4):
    type = 0x0146
    complex = lambda self: dyn.clone(IMsoArray, _object_=MSOPATHINFO, blocksize=lambda _: self.int())
...
class pConnectionSitesDir(uint4):
    type = 0x0152
    complex = lambda self: dyn.clone(IMsoArray, _object_=FixedPoint, blocksize=lambda _: self.int())
...
class pAdjustHandles(uint4):
    type = 0x0155
    complex = lambda self: dyn.clone(IMsoArray, _object_=ADJH, blocksize=lambda _: self.int())

@OfficeArtFOPTEOP.define
class pGuides(uint4):
    type = 0x0156
    complex = lambda self: dyn.clone(IMsoArray, _object_=SG, blocksize=lambda _: self.int())

@OfficeArtFOPTEOP.define
class pInscribe(uint4):
    type = 0x0157
    complex = lambda self: dyn.clone(IMsoArray, _object_=RECT, blocksize=lambda _: self.int())
...
class fillShadeColors(uint4):
    type = 0x0197
    complex = lambda self: dyn.clone(IMsoArray, _object_=MSOSHADECOLOR, blocksize=lambda _: self.int())
...
class lineDashStyle(uint4):
    type = 0x01cf
    complex = lambda self: dyn.clone(IMsoArray, _object_=uint4, blocksize=lambda _: self.int())
...
class gtextUNICODE(uint4):
    type = 0x00c0
    complex = lambda self: dyn.clone(pstr.wstring, length=self.int())
...
class pib(FixedPoint):
    type = 0x0104
    complex = lambda self: RecordGeneral
...
class pInkData(uint4):
    type = 0x0700
    #complex = lambda self: dyn.clone(pstr.wstring, length=self.int()) #FIXME
