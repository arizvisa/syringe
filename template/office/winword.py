...
class _Fts(pbinary.enum):
    length, _values_ = 3, Fts._values_[:]
...
class _Ipat(pbinary.enum):
    length, _values_ = 6, Ipat._values_[:-1] + [('iPatNil', 0x3f)]
...
class _BrcType(pbinary.enum):
    length, _values_ = 8, BrcType._values_[:]
...
class _Ico(pbinary.enum):
    length, _values_ = 5, Ico._values_
    def COLORREF(self):
...
class Sprm_Unpacked(pbinary.struct):
    ...
    class _spra(pbinary.enum):
        length, _values_ = 3, [(cls.__name__, ti) for ti, cls in SprmOperandType.cache.items()]
