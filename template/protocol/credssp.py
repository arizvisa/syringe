...
TSRequest = ber.Protocol.copy(recurse=True)
NegotiationData = ber.Protocol.copy(recurse=True)

class Packet(ber.Packet):
    Protocol = TSRequest
TSRequest.default = Packet

@TSRequest.Context.define
...
class NegoData(ber.SEQUENCE):
    tag = 1
    attributes = {'Protocol': NegotiationData}
...
@NegotiationData.Context.define
class NegoToken(ber.SEQUENCE):
    tag = 0
    _fields_ = [
        (nlmp.Message, 'NegoToken'),
    ]
